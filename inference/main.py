...
torch._C._jit_override_can_fuse_on_cpu(False)    # tensorexpr fuser crashes the fusion trace (range_check -1)
...
def main():
    ...
    mfiles = [f for f in sorted(glob.glob(f"{ASSETS}/*.ts.pt"))
              if not f.endswith(("ellipse.ts.pt", "ex_shared.ts.pt"))]
    models = [torch.jit.load(f, map_location=device).eval() for f in mfiles]
    if device == "cpu":
        models = [m.float() for m in models]
    # MIXED ROSTER (2026-09-11): ex_* members (e2c8x, plain) take the 2D PROJECTED anchor (union over
    # S-I, transforms.project_mask); fu_* members (attnres) take the 3D slabbed anchor. Same ellipse,
    # same masks -- only the reduction differs, exactly as at training time (train.py:99).
    is_ex = [os.path.basename(f).startswith("ex_") for f in mfiles]
    # e2c8x members ship as ONE shared traced graph (kernel-basis constants baked, steerable weights
    # live) + one small f32 state_dict per fold-model: the graph is re-loaded per member (page-cache
    # hot) and the member's weights are loaded into it. Verified equal to the eager models on real
    # boxes for all 50 fold-checkpoints (ex_shared_pack.py).
    if os.path.exists(f"{ASSETS}/ex_shared.ts.pt"):
        for sf in sorted(glob.glob(f"{ASSETS}/ex_*.sd.pt")):
            m = torch.jit.load(f"{ASSETS}/ex_shared.ts.pt", map_location=device).eval()
            if device == "cpu":
                m = m.float()
            m.load_state_dict(torch.load(sf, map_location=device))
            models.append(m); is_ex.append(True)
    ...
    @torch.no_grad()
    def predict(xb):
        """xb: (B,1,LR,AP,SI) NATIVE boxes -> canonicalize -> fusion ensemble logit -> calibrated p."""
        b0 = xb.shape[0]
        if b0 == 1:                                          # batch-1 guard: pad, drop after
            xb = torch.cat([xb, xb], 0)
        out = ell(xb.to(device).float())
        c, r, R = out[0], out[1], out[2]
        B = c.shape[0]
        ax = [torch.ones(n, device=c.device).cumsum(0) - 1.0 for n in (128, 128, 92)]
        gx, gy, gz = ax[0].view(1,1,-1,1,1), ax[1].view(1,1,1,-1,1), ax[2].view(1,1,1,1,-1)
        q = torch.zeros(B, 2, 128, 128, 92, device=c.device)
        for k in range(3):
            u = (R[:,:,k,0].view(B,2,1,1,1)*(gx-c[:,:,0].view(B,2,1,1,1))
                 + R[:,:,k,1].view(B,2,1,1,1)*(gy-c[:,:,1].view(B,2,1,1,1))
                 + R[:,:,k,2].view(B,2,1,1,1)*(gz-c[:,:,2].view(B,2,1,1,1)))
        ...
        an3 = CZ.with_slabs(mc)
        an2 = mc.amax(-1)                                     # (B,1,LR,AP) for the plain ex_* members
        zs = []
        for m, dt, ex in zip(models, dtypes, is_ex):
            anc = an2 if ex else an3
            z1 = m(xc.to(dt), anc.to(dt)).float()
            z2 = m(torch.flip(xc, dims=[2]).to(dt), torch.flip(anc, dims=[2]).to(dt)).float()
            z = (z1 + z2) / 2
            if CAP: z = z.clamp(-CAP, CAP)
            zs.append(z)
        z = torch.stack(zs, 0).mean(0)
        p = torch.sigmoid(a * z + b).clamp(1e-6, 1 - 1e-6).cpu().numpy()
        return p[:b0]
    ...
    with ProcessPoolExecutor(max_workers=NPROC) as pool:
        ...
        for k, batch in enumerate(batches):
            ...
            pb = predict(torch.from_numpy(np.stack([c[0] for c in cur]))[:, None])
            for i, pi, oki in zip(batch, pb, ok):
                probs[uids[i]] = float(pi) if oki else 0.5
